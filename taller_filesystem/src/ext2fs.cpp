...
{
	unsigned int nro_blockgroup = blockgroup_for_inode(inode_number);
	unsigned int inode_index = blockgroup_inode_index(inode_number);
	unsigned int block_size = 1024 << _superblock->log_block_size;

	unsigned char buffer[block_size];

	struct Ext2FSBlockGroupDescriptor* blockgroup_desc = block_group(nro_blockgroup);
	...
	memcpy(inodo, &buffer[posicion_inodo], _superblock->inode_size);
	return inodo;
}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	if(block_number < 12){
		// La direccion es alguna de las primeras 12
		block_address = inode->block[block_number];
	} else {

		unsigned int block_size = 1024 << _superblock->log_block_size; // Tamaño de bloque en bytes
		unsigned char buffer[block_size];
		unsigned int entradas_por_bloque = block_size / 4;

		if(block_number < 12 + entradas_por_bloque){
		// Indireccion simple
			read_block(inode->block[12], buffer); //  Copio al buffer el bloque de punteros datos.
			block_address = *((unsigned int*) &buffer[4 * (block_number - 12)]);
		} else if (block_number < 12 + entradas_por_bloque + (entradas_por_bloque * entradas_por_bloque)){
		// Indireccion doble
			read_block(inode->block[13], buffer); // Copio al buffer la primer tabla.
			block_address = *((unsigned int*) &buffer[4 * (block_number - 12 - entradas_por_bloque) / entradas_por_bloque]);

			read_block(block_address, buffer); // Copio al buffer el bloque de punteros a datos.
			block_address = *((unsigned int*) &buffer[4 * (block_number - 12 - entradas_por_bloque) % entradas_por_bloque]);
		} else {
		// Indireccion triple
			unsigned int entradas_anteriores = 12 + entradas_por_bloque + (entradas_por_bloque * entradas_por_bloque);
			read_block(inode->block[14], buffer); // Copio al buffer la primer tabla.

			block_address = *((unsigned int*) &buffer[4 * (block_number - entradas_anteriores) / (entradas_por_bloque * entradas_por_bloque)]);

			read_block(block_address, buffer); // Copio al buffer segunda tabla.
			block_address = *((unsigned int*) &buffer[4 * ((block_number - entradas_anteriores) / entradas_por_bloque) % entradas_por_bloque]);

			read_block(block_address, buffer); // Copio al buffer el bloque de punteros a datos.
			block_address = *((unsigned int*) &buffer[4 * (block_number - entradas_anteriores) % entradas_por_bloque]);
		}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	while(buffer_size > 7)
	{
		...
		record_length = (unsigned short) *(cursor + 4);
...
// unsigned int buscar_archivo(char bloque1&, char bloque2&, unsigned int offset&, const char* filename)
// {
// 	unsigned int block_size = 1024 << _superblock->log_block_size;
// 	char* cursor = bloque1 + offset;
// 	unsigned short record_length;

// 	// Mietras pueda leer el record_length
// 	while( block_size > 7 )
// 	{
// 		record_length = (unsigned short) *(cursor + 4);
// 		// Si puedo leer todo el nombre
// 		if (block_size >= record_length)
// 		{
// 			unsigned char name_length = (unsigned char) *(cursor + 6);
// 			// Comparo el nombre del archivo con el filename que me pasan
// 			if(strncmp(cursor + 7, filename, name_length) == 0)
// 			{
// 				return cursor;
// 			}
// 		}
// 		// Si no puedo leer todo el nombre, la dir entry esta partida. Lo analizo afuera.
// 		else
// 		{
// 			break;
// 		}
// 		block_size -= record_length;
// 		cursor += record_length;
// 	}
...
// 	if( block_size >= 7 ){
// 		record_length = (unsigned short) *(cursor + 4);
// 		offset = block_size - record_length;
// 		name_length = (unsigned char) *(cursor + 6);
// 		unsigned char first = name_length - block_size;
// 		char* skdh = cursor + first;
